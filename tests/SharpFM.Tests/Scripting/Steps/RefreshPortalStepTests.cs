...
{
    private const string CanonicalXml = """<Step enable="True" id="180" name="Refresh Portal"><ObjectName><Calculation><![CDATA[$x]]></Calculation></ObjectName></Step>""";

    [Fact]
    public void RoundTrip_CanonicalXml_IsPreserved()
    {
        var source = XElement.Parse(CanonicalXml);
        var step = RefreshPortalStep.Parse(source);
    ...
    {
        var step1 = RefreshPortalStep.Parse(XElement.Parse(CanonicalXml));
        ...
        var step2 = StepDisplayFactory.TryCreate(RefreshPortalStep.XmlName, true, tokens)!;
