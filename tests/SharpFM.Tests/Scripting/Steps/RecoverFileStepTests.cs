...
{
    private const string CanonicalXml = """<Step enable="True" id="95" name="Recover File"><NoInteract state="True"/><UniversalPathList>$example</UniversalPathList></Step>""";

    [Fact]
    public void RoundTrip_CanonicalXml_IsPreserved()
    {
        var source = XElement.Parse(CanonicalXml);
        var step = RecoverFileStep.Parse(source);
    ...
    {
        var step1 = RecoverFileStep.Parse(XElement.Parse(CanonicalXml));
        ...
        var step2 = StepDisplayFactory.TryCreate(RecoverFileStep.XmlName, true, tokens)!;
