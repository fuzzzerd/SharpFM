...
{
    private const string CanonicalXml = """<Step enable="True" id="200" name="Set Error Logging"><Option state="True"/><Calculation><![CDATA[$x]]></Calculation></Step>""";

    [Fact]
    public void RoundTrip_CanonicalXml_IsPreserved()
    {
        var source = XElement.Parse(CanonicalXml);
        var step = SetErrorLoggingStep.Parse(source);
    ...
    {
        var step1 = SetErrorLoggingStep.Parse(XElement.Parse(CanonicalXml));
        ...
        var step2 = StepDisplayFactory.TryCreate(SetErrorLoggingStep.XmlName, true, tokens)!;
