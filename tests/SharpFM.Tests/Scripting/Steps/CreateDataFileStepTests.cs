...
{
    private const string CanonicalXml = """<Step enable="True" id="190" name="Create Data File"><UniversalPathList>$example</UniversalPathList><CreateDirectories state="True"/></Step>""";

    [Fact]
    public void RoundTrip_CanonicalXml_IsPreserved()
    {
        var source = XElement.Parse(CanonicalXml);
        var step = CreateDataFileStep.Parse(source);
    ...
    {
        var step1 = CreateDataFileStep.Parse(XElement.Parse(CanonicalXml));
        ...
        var step2 = StepDisplayFactory.TryCreate(CreateDataFileStep.XmlName, true, tokens)!;
