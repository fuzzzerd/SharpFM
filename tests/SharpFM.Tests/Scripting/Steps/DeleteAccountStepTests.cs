...
{
    private const string CanonicalXml = """<Step enable="True" id="135" name="Delete Account"><AccountName><Calculation><![CDATA[$x]]></Calculation></AccountName></Step>""";

    [Fact]
    public void RoundTrip_CanonicalXml_IsPreserved()
    {
        var source = XElement.Parse(CanonicalXml);
        var step = DeleteAccountStep.Parse(source);
    ...
    {
        var step1 = DeleteAccountStep.Parse(XElement.Parse(CanonicalXml));
        ...
        var step2 = StepDisplayFactory.TryCreate(DeleteAccountStep.XmlName, true, tokens)!;
