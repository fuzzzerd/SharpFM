...
{
    private const string CanonicalXml = """<Step enable="True" id="96" name="Save a Copy as Add-on Package"><LinkAvail state="True"/><Calculation><![CDATA[$x]]></Calculation></Step>""";

    [Fact]
    public void RoundTrip_CanonicalXml_IsPreserved()
    {
        var source = XElement.Parse(CanonicalXml);
        var step = SaveACopyAsAddOnPackageStep.Parse(source);
    ...
    {
        var step1 = SaveACopyAsAddOnPackageStep.Parse(XElement.Parse(CanonicalXml));
        ...
        var step2 = StepDisplayFactory.TryCreate(SaveACopyAsAddOnPackageStep.XmlName, true, tokens)!;
