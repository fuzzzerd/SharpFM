...
{
    private const string CanonicalXml = """<Step enable="True" id="111" name="Open URL"><NoInteract state="True"/><Option state="True"/><Calculation><![CDATA[$x]]></Calculation></Step>""";

    [Fact]
    public void RoundTrip_CanonicalXml_IsPreserved()
    {
        var source = XElement.Parse(CanonicalXml);
        var step = OpenURLStep.Parse(source);
    ...
    {
        var step1 = OpenURLStep.Parse(XElement.Parse(CanonicalXml));
        ...
        var step2 = StepDisplayFactory.TryCreate(OpenURLStep.XmlName, true, tokens)!;
