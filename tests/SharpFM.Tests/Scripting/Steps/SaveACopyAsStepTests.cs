...
{
    private const string CanonicalXml = """<Step enable="True" id="37" name="Save a Copy as"><CreateDirectories state="True"/><AutoOpen state="True"/><CreateEmail state="True"/><SaveAsType value="Copy"/><UniversalPathList>$example</UniversalPathList></Step>""";

    [Fact]
    public void RoundTrip_CanonicalXml_IsPreserved()
    {
        var source = XElement.Parse(CanonicalXml);
        var step = SaveACopyAsStep.Parse(source);
    ...
    {
        var step1 = SaveACopyAsStep.Parse(XElement.Parse(CanonicalXml));
        ...
        var step2 = StepDisplayFactory.TryCreate(SaveACopyAsStep.XmlName, true, tokens)!;
