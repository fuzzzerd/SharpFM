...
{
    private const string CanonicalXml = """<Step enable="True" id="199" name="Rename File"><UniversalPathList>$example</UniversalPathList><Calculation><![CDATA[$x]]></Calculation></Step>""";

    [Fact]
    public void RoundTrip_CanonicalXml_IsPreserved()
    {
        var source = XElement.Parse(CanonicalXml);
        var step = RenameFileStep.Parse(source);
    ...
    {
        var step1 = RenameFileStep.Parse(XElement.Parse(CanonicalXml));
        ...
        var step2 = StepDisplayFactory.TryCreate(RenameFileStep.XmlName, true, tokens)!;
