...
{
    private const string CanonicalXml = """<Step enable="True" id="123" name="Select Window"><LimitToWindowsOfCurrentFile state="True"/><Window value="ByName"/><Name><Calculation><![CDATA[$x]]></Calculation></Name></Step>""";

    [Fact]
    public void RoundTrip_CanonicalXml_IsPreserved()
    {
        var source = XElement.Parse(CanonicalXml);
        var step = SelectWindowStep.Parse(source);
    ...
    {
        var step1 = SelectWindowStep.Parse(XElement.Parse(CanonicalXml));
        ...
        var step2 = StepDisplayFactory.TryCreate(SelectWindowStep.XmlName, true, tokens)!;
