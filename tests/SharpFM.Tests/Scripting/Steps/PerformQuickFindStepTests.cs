...
{
    private const string CanonicalXml = """<Step enable="True" id="150" name="Perform Quick Find"><Calculation><![CDATA[$x]]></Calculation></Step>""";

    [Fact]
    public void RoundTrip_CanonicalXml_IsPreserved()
    {
        var source = XElement.Parse(CanonicalXml);
        var step = PerformQuickFindStep.Parse(source);
    ...
    {
        var step1 = PerformQuickFindStep.Parse(XElement.Parse(CanonicalXml));
        ...
        var step2 = StepDisplayFactory.TryCreate(PerformQuickFindStep.XmlName, true, tokens)!;
